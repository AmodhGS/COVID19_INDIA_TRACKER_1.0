...
class MainActivity : AppCompatActivity() {
    ...
    private fun bindData(data: StatewiseItem) {
        val last_updated = data.lastupdatedtime
        val date_format = SimpleDateFormat("dd/MM/yy HH:mm:ss")
        //update the time

        last_update.text = "LAST UPDATED\n ${getTimeAgo(date_format.parse(last_updated))}"

        //connecting attributes in container
        confirmed.text=data.confirmed
        active.text=data.active
        recovered_pat.text=data.recovered
